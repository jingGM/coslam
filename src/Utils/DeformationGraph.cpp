...
void DeformationGraph::setPosesSeq(std::vector<unsigned long long int> * poseTimeMap, const std::vector<Eigen::Matrix4f> & poses)
{
    ...
    for(unsigned int i = 0; i < poses.size(); i++)
    {
        ...
        //find the nearest last lookBack=20 nodes.
        unsigned int distanceBack = 0;
        for(int j = (int)foundIndex; j >= 0; j--)
        {
            std::pair<float, int> newNode;
            // get the normed distance in the world frame between the current node i and the previous node j
        ...
        // if the previous nodes are not enough, get the following nodes
        if(distanceBack != lookBack)
        {
        ...
        // give all nearest 4 nodes weights
        for(unsigned int j = 0; j < (unsigned int)k; j++)
        {
            // the weight depends on the distance to the node, the near the heavier
        ...
        // sort the newMap using timeStamps instead of the distance
...
bool DeformationGraph::optimiseGraphSparse(float & error, float & meanConsErr, const bool fernMatch, const unsigned long long int lastDeformTime)
{
    ...
    // accumulation of the differences in relative constraints between target and source points with current translation
    ...
    int numCols = 0;                            // all vertices after lastDeformTime
    int backSet = graph.size() * numVariables;  // all vertices before lastDeformTime
