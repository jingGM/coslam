...
: indexProgram(loadProgramFromFile("index_map.vert", "index_map.frag")),
  indexRenderBuffer(LocalCameraInfo::getInstance().width() * IndexMap::FACTOR,
                    LocalCameraInfo::getInstance().height() * IndexMap::FACTOR),
  indexTexture(LocalCameraInfo::getInstance().width() * IndexMap::FACTOR,
               LocalCameraInfo::getInstance().height() * IndexMap::FACTOR,
               GL_LUMINANCE32UI_EXT,
               GL_LUMINANCE_INTEGER_EXT,
               GL_UNSIGNED_INT),
  vertConfTexture(LocalCameraInfo::getInstance().width() * IndexMap::FACTOR,
                  LocalCameraInfo::getInstance().height() * IndexMap::FACTOR,
                  GL_RGBA32F, GL_LUMINANCE, GL_FLOAT),
  colorTimeTexture(LocalCameraInfo::getInstance().width() * IndexMap::FACTOR,
                   LocalCameraInfo::getInstance().height() * IndexMap::FACTOR,
                   GL_RGBA32F, GL_LUMINANCE, GL_FLOAT),
  normalRadTexture(LocalCameraInfo::getInstance().width() * IndexMap::FACTOR,
                   LocalCameraInfo::getInstance().height() * IndexMap::FACTOR,
                   GL_RGBA32F, GL_LUMINANCE, GL_FLOAT),
  drawDepthProgram(loadProgramFromFile("empty.vert", "visualise_textures.frag", "quad.geom")),
  drawRenderBuffer(LocalCameraInfo::getInstance().width(),
                   LocalCameraInfo::getInstance().height()),
  drawTexture(LocalCameraInfo::getInstance().width(),
              LocalCameraInfo::getInstance().height(),
              GL_RGBA,
              GL_RGB,
              GL_UNSIGNED_BYTE,
  ...
  depthRenderBuffer(LocalCameraInfo::getInstance().width(), LocalCameraInfo::getInstance().height()),
  depthTexture(LocalCameraInfo::getInstance().width(),
               LocalCameraInfo::getInstance().height(),
               GL_LUMINANCE32F_ARB,
               GL_LUMINANCE,
               GL_FLOAT,
  ...
  combinedRenderBuffer(LocalCameraInfo::getInstance().width(), LocalCameraInfo::getInstance().height()),
  ...
  normalTexture(LocalCameraInfo::getInstance().width(),
                LocalCameraInfo::getInstance().height(),
                GL_RGBA32F, GL_LUMINANCE, GL_FLOAT, false, true),
  timeTexture(LocalCameraInfo::getInstance().width(),
              LocalCameraInfo::getInstance().height(),
              ...
              false,
              true),
  oldRenderBuffer(LocalCameraInfo::getInstance().width(), LocalCameraInfo::getInstance().height()),
  oldImageTexture(LocalCameraInfo::getInstance().width(),
                  LocalCameraInfo::getInstance().height(),
                  GL_RGBA,
                  GL_RGB,
                  GL_UNSIGNED_BYTE,
                  false,
                  true),
  oldVertexTexture(LocalCameraInfo::getInstance().width(),
                   LocalCameraInfo::getInstance().height(),
                   GL_RGBA32F, GL_LUMINANCE, GL_FLOAT, false, true),
  oldNormalTexture(LocalCameraInfo::getInstance().width(),
                   LocalCameraInfo::getInstance().height(),
                   GL_RGBA32F, GL_LUMINANCE, GL_FLOAT, false, true),
  oldTimeTexture(LocalCameraInfo::getInstance().width(),
                 LocalCameraInfo::getInstance().height(),
                 GL_LUMINANCE16UI_EXT,
                 GL_LUMINANCE_INTEGER_EXT,
                 GL_UNSIGNED_SHORT,
                 false,
                 true),
  infoRenderBuffer(LocalCameraInfo::getInstance().width(), LocalCameraInfo::getInstance().height()),
  colorInfoTexture(LocalCameraInfo::getInstance().width() * IndexMap::FACTOR,
                   LocalCameraInfo::getInstance().height() * IndexMap::FACTOR,
                   GL_RGBA32F, GL_LUMINANCE, GL_FLOAT),
  vertexInfoTexture(LocalCameraInfo::getInstance().width() * IndexMap::FACTOR,
                   LocalCameraInfo::getInstance().height() * IndexMap::FACTOR,
                   GL_RGBA32F, GL_LUMINANCE, GL_FLOAT),
  normalInfoTexture(LocalCameraInfo::getInstance().width() * IndexMap::FACTOR,
                    LocalCameraInfo::getInstance().height() * IndexMap::FACTOR,
                    GL_RGBA32F, GL_LUMINANCE, GL_FLOAT)
{
                              ...
                              const int timeDelta)
{
    ...
    Eigen::Vector4f cam(LocalCameraInfo::getInstance().cx() * IndexMap::FACTOR,
                  LocalCameraInfo::getInstance().cy() * IndexMap::FACTOR,
                  LocalCameraInfo::getInstance().fx() * IndexMap::FACTOR,
                  LocalCameraInfo::getInstance().fy() * IndexMap::FACTOR);
    ...
    indexProgram->setUniform(Uniform("cols", (float)LocalCameraInfo::getInstance().cols() * IndexMap::FACTOR));
    indexProgram->setUniform(Uniform("rows", (float)LocalCameraInfo::getInstance().rows() * IndexMap::FACTOR));
                               ...
                               IndexMap::Prediction predictionType)
{
    ...
    combinedProgram->setUniform(Uniform("cols", (float)LocalCameraInfo::getInstance().cols()));
    combinedProgram->setUniform(Uniform("rows", (float)LocalCameraInfo::getInstance().rows()));
                               ...
                               const int timeDelta)
{
    ...
    depthProgram->setUniform(Uniform("cols", (float)LocalCameraInfo::getInstance().cols()));
    depthProgram->setUniform(Uniform("rows", (float)LocalCameraInfo::getInstance().rows()));
