...
GUI::GUI(bool liveCap, bool showcaseMode)
...
void GUI::drawFrustum(const Eigen::Matrix4f & pose)
{
    ...
    K(0, 0) = LocalCameraInfo::getInstance().fx();
    K(1, 1) = LocalCameraInfo::getInstance().fy();
    K(0, 2) = LocalCameraInfo::getInstance().cx();
    K(1, 2) = LocalCameraInfo::getInstance().cy();

    Eigen::Matrix3f Kinv = K.inverse();

    pangolin::glDrawFrustum(Kinv,
                            LocalCameraInfo::getInstance().width(),
              ...
              const bool invertNormals)
{
    ...
    Eigen::Vector2f LocalCameraInfo(renderBuffer->width, renderBuffer->height);

    fxaaProgram->setUniform(Uniform("tex", 0));
    fxaaProgram->setUniform(Uniform("LocalCameraInfo", LocalCameraInfo));
