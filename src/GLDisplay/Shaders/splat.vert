...
void main()
{
    ...
    {
        // the normalized coordinate in image plane, where the center is the origin
	    gl_Position = vec4(projectPoint(vPosHome.xyz), 1.0);
	    
        colTime = vColor;
	    position = vec4(vPosHome.xyz, vPosition.w);
        // normal vector is converted to the world frame and normalized
	    normRad = vec4(normalize(mat3(t_inv) * vNormRad.xyz), vNormRad.w);
	    
        // normRad.w * 1.4 ~~ depth/focal_mean
