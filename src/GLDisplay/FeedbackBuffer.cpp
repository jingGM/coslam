...
FeedbackBuffer::FeedbackBuffer(std::shared_ptr<Shader> program)
 : program(program),
   ...
   bufferSize(LocalCameraInfo::getInstance().numPixels() * Vertex::SIZE),
   count(0)
{
    float * vertices = new float[bufferSize];

    memset(&vertices[0], 0, bufferSize); // (pixel# * 12) bytes
    ...
    glBufferData(GL_ARRAY_BUFFER, uv.size() * sizeof(Eigen::Vector2f), &uv[0], GL_STATIC_DRAW); // (2*pixel) bytes
    glBindBuffer(GL_ARRAY_BUFFER, 0);
                             ...
                             const float depthCutoff)
{
    ...
    program->setUniform(Uniform("cols", (float)LocalCameraInfo::getInstance().cols()));
    program->setUniform(Uniform("rows", (float)LocalCameraInfo::getInstance().rows()));
    ...
    glDrawArrays(GL_POINTS, 0, LocalCameraInfo::getInstance().numPixels());
