...
GlobalModel::GlobalModel()
 : target(0),
   ...
   count(0),
   initProgram(loadProgramFromFile("init_unstable.vert")), // copy the values to vertex buffer
   ...
   deformationNodes(NODE_TEXTURE_DIMENSION, 1, GL_LUMINANCE32F_ARB, GL_LUMINANCE, GL_FLOAT)
{
    ...
    vertices = new float[LocalCameraInfo::getInstance().numPixels() * Vertex::SIZE];

    memset(&vertices[0], 0, LocalCameraInfo::getInstance().numPixels() * Vertex::SIZE);

    glGenTransformFeedbacks(1, &newUnstableFid);
    glGenBuffers(1, &newUnstableVbo);
    glBindBuffer(GL_ARRAY_BUFFER, newUnstableVbo);
    glBufferData(GL_ARRAY_BUFFER, LocalCameraInfo::getInstance().numPixels() * Vertex::SIZE, &vertices[0], GL_STREAM_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    ...
    for(int i = 0; i < LocalCameraInfo::getInstance().width(); i++)
    {
        for(int j = 0; j < LocalCameraInfo::getInstance().height(); j++)
        {
            uv.push_back(Eigen::Vector2f(((float)i / (float)LocalCameraInfo::getInstance().width()) + 1.0 / (2 * (float)LocalCameraInfo::getInstance().width()),
                                   ((float)j / (float)LocalCameraInfo::getInstance().height()) + 1.0 / (2 * (float)LocalCameraInfo::getInstance().height())));
        }
    }
                       ...
                       const float weighting)
{
    ...
    // put the vertices need to be updated into newUnstableVbo which has the same size with indexmap
    frameBuffer.Bind();
...
//    Eigen::Matrix4f t_inv = pose;
    ...
    dataProgram->setUniform(Uniform("cam", Eigen::Vector4f(LocalCameraInfo::getInstance().cx(),
                                                     LocalCameraInfo::getInstance().cy(),
                                                     1.0 / LocalCameraInfo::getInstance().fx(),
                                                     1.0 / LocalCameraInfo::getInstance().fy())));
    dataProgram->setUniform(Uniform("cols", (float)LocalCameraInfo::getInstance().cols()));
    dataProgram->setUniform(Uniform("rows", (float)LocalCameraInfo::getInstance().rows()));
    ...
    // position, color, norm are all updated by weight(confidence)
    //Using a transform feedback conditional on a texture sample
    // save them to rendersource and rendered in target buffer, then exchange the two.
    updateProgram->Bind();
                        ...
                        const bool isFern)
{
    ...
    unstableProgram->setUniform(Uniform("cam", Eigen::Vector4f(LocalCameraInfo::getInstance().cx(),
                                                         LocalCameraInfo::getInstance().cy(),
                                                         LocalCameraInfo::getInstance().fx(),
                                                         LocalCameraInfo::getInstance().fy())));
    unstableProgram->setUniform(Uniform("cols", (float)LocalCameraInfo::getInstance().cols()));
    unstableProgram->setUniform(Uniform("rows", (float)LocalCameraInfo::getInstance().rows()));
