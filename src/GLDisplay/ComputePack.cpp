...
ComputePack::ComputePack(std::shared_ptr<Shader> program,
 ...
 : program(program),
   renderBuffer(LocalCameraInfo::getInstance().width(), LocalCameraInfo::getInstance().height()),
   target(target)
{
    frameBuffer.AttachColour(*target); // attach color to framebuffer
    frameBuffer.AttachDepth(renderBuffer); // attach depth to framebuffer
}
