                            ...
                            std::vector<Constraint> * newRelativeCons)
{
    if(def.isInit())
    {
        ...
        // for each pose, look back 20 nodes from the last time sampled graphCloud in time sequence
        // and choose the nearest k=4 nodes in distance and put them in time order in poseMap
        ...
        // for each vertexTime, look back 20 vertices from pointPool/def.sourceVertices in time sequence
        // and choose the nearest k=4 vertices in distance and put them in time order in vertexMap
        ...
        // optimize the graph of rotation and translation using the neighborhoods by constraints
...
void Deformation::sampleGraphModel(const std::pair<GLuint, GLuint> & model)
{
    ...
    if((int)count > def.k)
    {
        ...
        // set all neighborhoods to the def: +- k/2
        def.initialiseGraph(graphPosePoints, &graphPoseTimes);
