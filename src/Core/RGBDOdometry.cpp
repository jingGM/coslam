...
// copy  the filteredDepth to depth_tmp[0] and use Guassian prramid to make pyramids
...
// copy vertices to vmaps_tmp and conver to global vmaps_g_prev_
// copy normals to nmaps_tmp and conver to global nmaps_g_prev_
                                ...
                                const Eigen::Matrix4f & modelPose)
{
    ...
    // convert vmaps_tmp (height * 4 * width): {[(0,1,2,3),(0,1,2,3)...],[...]}
    // to vmaps_g_prev_ x: (0-width, height), y:(width-2width, height), z:(2width-3width, height)
...
// copy depth(vmaps_tmp.z) to lastDepth, rgb.cudares.texture to lastImage
 ...
 * if so3: use the level=2, use lastNextImage and nextImage to optimize resultR and then give resultR to resultRt.
 * for all pyramid
      if rgb: give lastDepth to pointClouds(x,y,z in local frame)

      use vmap_curr, nmap_curr, vmap_g_prev, nmap_g_prev in optimization factor of icp to get sumDataSE3, residual, A_icp, b_icp
      use nextdIdx, nextdIdy, lastDepth, nextDepth, lastImage and nextImage in optimization factor of rgb to get corresImg, sumResidualRGB, sigma. Then use sumDataSE3, outDataSE3, corresImg, nextdIdx, nextdIdy to get A_rgbd, b_rgbd

      give weights to last two factors, and optimize tcurr and Rcurr
 **/
