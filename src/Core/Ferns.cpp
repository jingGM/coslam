...
Ferns::Ferns(int n, int maxDepth, const float photoThresh)
 : num(n), // 500 here
   factor(8),
   width(LocalCameraInfo::getInstance().width() / factor),
   height(LocalCameraInfo::getInstance().height() / factor),
   maxDepth(maxDepth),
   photoThresh(photoThresh),
   minFrameTimeDist(300),
   minHDAwareThresh(0.3),
   widthDist(0, width - 1),
   ...
   lastClosest(-1),
   badCode(255),
   rgbd(LocalCameraInfo::getInstance().width() / factor,
        LocalCameraInfo::getInstance().height() / factor,
        LocalCameraInfo::getInstance().cx() / factor,
        LocalCameraInfo::getInstance().cy() / factor,
        LocalCameraInfo::getInstance().fx() / factor,
        LocalCameraInfo::getInstance().fy() / factor),
   ...
   resize(LocalCameraInfo::getInstance().width(),
          LocalCameraInfo::getInstance().height(), width, height),
                     ...
                     const float threshold)
{
    ...
    for(int i = 0; i < num; i++)
    {
        ...
        // conservatory has random points, so the pos is also random sampled
        if(verts.at<Eigen::Vector4f>(conservatory.at(i).pos(1), conservatory.at(i).pos(0))(2) > 0)
        {
            const Eigen::Matrix<unsigned char, 3, 1> & pix = img.at<Eigen::Matrix<unsigned char, 3, 1>>(conservatory.at(i).pos(1),
                                                                                                        conservatory.at(i).pos(0));

            // compare the sampled rgbd with conservatory rgbd
            code = (pix(0) > conservatory.at(i).rgbd(0)) << 3 |
                   ...
                   (int(verts.at<Eigen::Vector4f>(conservatory.at(i).pos(1),
                                                  conservatory.at(i).pos(0))(2) * 1000.0f) > conservatory.at(i).rgbd(3));
        ...
        // codes stores the code of all ferns with the sampled vertex
        // code is the index of ferns.ids
        frame->codes[i] = code;
    }
                                 ...
                                 const bool lost)
{
    ...
    for(int i = 0; i < num; i++)
    {
        ...
        // conservatory is randomized num Ferns
        // If the vertSmall pixel in the ith conservatory's pose has depth
        if(vertSmall.at<Eigen::Vector4f>(conservatory.at(i).pos(1),conservatory.at(i).pos(0))(2) > 0)
        {
            const Eigen::Matrix<unsigned char, 3, 1> & pix =
                    imgSmall.at<Eigen::Matrix<unsigned char, 3, 1>>(conservatory.at(i).pos(1),
                                                                    conservatory.at(i).pos(0));
...
//            std::cout<< conservatory.at(i).ids[code].size()<< std::endl;
            for(size_t j = 0; j < conservatory.at(i).ids[code].size(); j++)
            {
                // ids: [16, n]: code index, n values;  the values are indices of coOccurrences
                // plus one to all those n values indices in coOccurences.
                coOccurrences[conservatory.at(i).ids[code].at(j)]++;
            }
        }

        // defined the relation between the current frame and the existing frames
        frame->codes[i] = code;
    }

    // minId=most similar id and minimum=1 - most similar value
    float minimum = std::numeric_limits<float>::max();
    int minId = -1;
    ...
    for(size_t i = 0; i < frames.size(); i++)
    {
        ...
        if(dissim < minimum && time - frames.at(i)->srcTime > minFrameTimeDist)
        {
            minimum = dissim;
            minId = i;
        }
    ...
    if(minId != -1 && blockHDAware(frame, frames.at(minId)) > minHDAwareThresh)
    {
        ...
        if(rgbd.lastICPError < 0.0003 && rgbd.lastICPCount > icpCountThresh && photoError < photoThresh)
        {
            ...
            for(int i = 0; i < num; i += num / 50)
            {
                if(vertSmall.at<Eigen::Vector4f>(conservatory.at(i).pos(1), conservatory.at(i).pos(0))(2) > 0 &&
                   int(vertSmall.at<Eigen::Vector4f>(conservatory.at(i).pos(1),
                                                     conservatory.at(i).pos(0))(2) * 1000.0f) < maxDepth)
                {
...
{
    float cx = LocalCameraInfo::getInstance().cx() / factor;
    float cy = LocalCameraInfo::getInstance().cy() / factor;
    float invfx = 1.0f / float(LocalCameraInfo::getInstance().fx() / factor);
    float invfy = 1.0f / float(LocalCameraInfo::getInstance().fy() / factor);
    ...
    // plus all differences of rgb values between 500 sampled vertices and the vertSmall
    // while the vertSmall has the points in the fern camera plane
    for(int i = 0; i < num; i++)
    {
        if(vertSmall.at<Eigen::Vector4f>(conservatory.at(i).pos(1), conservatory.at(i).pos(0))(2) > 0 &&
