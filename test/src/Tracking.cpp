...
Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, const string &strSettingPath, bool debug):
        mState(NO_IMAGES_YET),mpORBVocabulary(pVoc),mpSystem(pSys), debug(debug) {
...
void Tracking::initializeORBPrams(const cv::FileStorage &fSettings) {
    moptimizeIterations = fSettings["Initializer.iterations"];
    msigma = fSettings["Initializer.sigma"];
    minParallax = fSettings["Initializer.minParallax"];
    minTriangulated = fSettings["Initializer.minTriangulated"];
    bestNumberRate = fSettings["Initializer.bestNumberRate"];
    BestToSecondBest = fSettings["Initializer.BestToSecondBest"];
    RHThreshold = fSettings["Initializer.RHThreshold"];
    ...
    double globalFeatureNumberFactor = fSettings["ORBextractor.globalFeatureNumberFactor"];
    int globalFeatureNumber = round(globalFeatureNumberFactor* nFeatures);
    mpORBextractorLeft = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
    mpIniORBextractor = new ORBextractor(globalFeatureNumber,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
...
cv::Mat Tracking::processImage(const cv::Mat &im, const double &timestamp, const bool& isGlobalFrame, bool useBoW) {
    mImGray = im;
    mImOrigin = im;
    ...
    matchFrames(isGlobalFrame, useBoW);
    return mCurrentFrame.mTcw.clone();
}

void Tracking::matchFrames(const bool& isGlobalFrame, bool useBoW) {
    if (isGlobalFrame) {
        initializeGlobal();
    } else {
...
void Tracking::initializeGlobal() {
    ...
    mpInitializer =  new Initializer(mCurrentFrame,debug,msigma,moptimizeIterations,
                                     minParallax,minTriangulated,bestNumberRate,BestToSecondBest,RHThreshold);
...
void Tracking::processLocal(bool useBoW) {
